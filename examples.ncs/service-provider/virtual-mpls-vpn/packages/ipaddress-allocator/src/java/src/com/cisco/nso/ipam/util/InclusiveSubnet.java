...
public class InclusiveSubnet extends Subnet {
    ...
    /**
     * the number of addresses available in the subnet, not counting
     ...
     *
     * @return Actual number of addresses in the subnet block.
     */
    @Override
    public long size() {
        if (isSingleHost()) {
    ...
    public InclusiveSubnet[] split()
        ...
        if (size() > 1) {
            ...
            result[1] = new InclusiveSubnet(InetAddress.
                                            getByAddress(addr.getData()),
                                            getCIDRMask() + 1);
            return result;
        }
