...
#ifndef MAXDEPTH
#define MAXDEPTH 60   /* max depth of model tree (max KP length + 1) */
#endif
#ifndef MAXKEYLEN
#define MAXKEYLEN 18   /* max number of key elems (max keys + 1) */
#endif
...
#define CONFD_LIB_VSN           0x07070100
#define CONFD_LIB_VSN_STR       "07070100"
#define CONFD_LIB_API_VSN       0x07070000
#define CONFD_LIB_API_VSN_STR   "07070000"
#define CONFD_LIB_PROTO_VSN     77
#define CONFD_LIB_PROTO_VSN_STR "77"
...
/* Error info for type CONFD_ERRTYPE_NCS */
...
#define CONFD_TRANS_CB_FLAG_FILTERED (1 << 0)
...
enum confd_iter_flags {     /* bitmask */
    ...
    ITER_WANT_CLI_ORDER = (1<<10)        /* cdb_get_modifications_cli() only */
};
...
enum confd_progress_verbosity {
     ...
     * operations.
     */
    CONFD_VERBOSITY_DEBUG        = 3
};

enum confd_progress_event_type {
    CONFD_PROGRESS_START = 1, /* an event is started       */
    CONFD_PROGRESS_STOP  = 2, /* an event has stopped      */
    CONFD_PROGRESS_INFO  = 3  /* single event without span */
};
...
extern enum confd_vtype confd_get_base_type(struct confd_cs_node *node);
...
#define CONFD_GET_XMLMOVEFIRST(_v) \
    (CONFD_ASSERT((_v)->type == C_XMLMOVEFIRST), (_v)->val.xmltag.tag)
#define CONFD_GET_XMLMOVEFIRST_NS(_v) \
    (CONFD_ASSERT((_v)->type == C_XMLMOVEFIRST), (_v)->val.xmltag.ns)
#define CONFD_GET_XMLMOVEAFTER(_v) \
    (CONFD_ASSERT((_v)->type == C_XMLMOVEAFTER), (_v)->val.xmltag.tag)
#define CONFD_GET_XMLMOVEAFTER_NS(_v) \
    (CONFD_ASSERT((_v)->type == C_XMLMOVEAFTER), (_v)->val.xmltag.ns)
#define CONFD_GET_XML(_v) \
    (CONFD_ASSERT((_v)->type == C_XMLTAG || (_v)->type == C_XMLBEGIN || \
                  (_v)->type == C_XMLBEGINDEL || (_v)->type == C_XMLEND || \
                  (_v)->type == C_XMLMOVEFIRST || \
                  (_v)->type == C_XMLMOVEAFTER), \
     (_v)->val.xmltag.tag)
...
#define CONFD_GET_TAG_ATTR_VALUE(_v) (&(_v)->attrs->v)


/* A set of macros to set the value and attrs of confd_tag_value_attr_t */

#define CONFD_SET_TAG_ATTR_STR(_v, _t_tag, _str, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                       \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);              \
        __tvap->attrs = _attrs; __tvap->num_attrs = (_num_attrs);    \
        CONFD_SET_STR(&__tvap->v, (_str));} while(0)
#define CONFD_SET_TAG_ATTR_BUF(_v, _t_tag, _bufp, _len, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                              \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                     \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;             \
        CONFD_SET_BUF(&__tvap->v, (_bufp), (_len));} while(0)
/* a version that works with signed char* */
#define CONFD_SET_TAG_ATTR_CBUF(_v, _t_tag, _bufp, _len, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                               \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                      \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;              \
        CONFD_SET_CBUF(&__tvap->v, (_bufp), (_len));} while(0)
#define CONFD_SET_TAG_ATTR_BINARY(_v, _t_tag, _bufp, _len, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                                 \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                        \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;                \
        CONFD_SET_BINARY(&__tvap->v, (_bufp), (_len));} while(0)
#define CONFD_SET_TAG_ATTR_IPV4(_v, _t_tag, _ipval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                          \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                 \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;         \
        CONFD_SET_IPV4(&__tvap->v, (_ipval));} while(0)
#define CONFD_SET_TAG_ATTR_IPV6(_v, _t_tag, _ipval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                          \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                 \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;         \
        CONFD_SET_IPV6(&__tvap->v, (_ipval));} while(0)

#define CONFD_SET_TAG_ATTR_INT8(_v, _t_tag, _i, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                      \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);             \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;     \
        CONFD_SET_INT8(&__tvap->v, (_i));} while(0)
#define CONFD_SET_TAG_ATTR_INT16(_v, _t_tag, _i, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                       \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);              \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;      \
        CONFD_SET_INT16(&__tvap->v, (_i));} while(0)
#define CONFD_SET_TAG_ATTR_INT32(_v, _t_tag, _i, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                       \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);              \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;      \
        CONFD_SET_INT32(&__tvap->v, (_i));} while(0)
#define CONFD_SET_TAG_ATTR_INT64(_v, _t_tag, _i, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                       \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);              \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;      \
        CONFD_SET_INT64(&__tvap->v, (_i));} while(0)
#define CONFD_SET_TAG_ATTR_BOOL(_v, _t_tag, _b, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                      \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);             \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;     \
        CONFD_SET_BOOL(&__tvap->v, (_b));} while(0)

#define CONFD_SET_TAG_ATTR_UINT8(_v, _t_tag, _ui, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                        \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);               \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;       \
        CONFD_SET_UINT8(&__tvap->v, (_ui));} while(0)
#define CONFD_SET_TAG_ATTR_UINT16(_v, _t_tag, _ui, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                         \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;        \
        CONFD_SET_UINT16(&__tvap->v, (_ui));} while(0)
#define CONFD_SET_TAG_ATTR_UINT32(_v, _t_tag, _ui, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                         \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;        \
        CONFD_SET_UINT32(&__tvap->v, (_ui));} while(0)
#define CONFD_SET_TAG_ATTR_UINT64(_v, _t_tag, _ui, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                         \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;        \
        CONFD_SET_UINT64(&__tvap->v, (_ui));} while(0)

#define CONFD_SET_TAG_ATTR_DOUBLE(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                           \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                  \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;          \
        CONFD_SET_DOUBLE(&__tvap->v, (_dval));} while(0)

#define CONFD_SET_TAG_ATTR_QNAME(_v, _t_tag, _pre, _presz, _n,           \
                                 _nsz, _attrs, _num_attrs)               \
    do {confd_tag_value_attr_t *__tvap = (_v);                           \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                  \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;          \
        CONFD_SET_QNAME(&__tvap->v, (_pre), (_presz), (_n), (_nsz));} while(0)

#define CONFD_SET_TAG_ATTR_DATETIME(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                             \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                    \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;            \
        CONFD_SET_DATETIME(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_DATE(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                         \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;        \
        CONFD_SET_DATE(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_TIME(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                         \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;        \
        CONFD_SET_TIME(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_DURATION(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                             \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                    \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;            \
        CONFD_SET_DURATION(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_ENUM_VALUE(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                               \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                      \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;              \
        CONFD_SET_ENUM_VALUE(&__tvap->v, (_dval));} while(0)
/* for backwards compatibility */
#define CONFD_SET_TAG_ATTR_ENUM_HASH(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                              \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                     \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;             \
        CONFD_SET_ENUM_HASH(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_BIT32(_v, _t_tag, _ui, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                        \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);               \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;       \
        CONFD_SET_BIT32(&__tvap->v, (_ui));} while(0)
#define CONFD_SET_TAG_ATTR_BIT64(_v, _t_tag, _ui, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                        \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);               \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;       \
        CONFD_SET_BIT64(&__tvap->v, (_ui));} while(0)
#define CONFD_SET_TAG_ATTR_BITBIG(_v, _t_tag, _bufp, _len, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                                 \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                        \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;                \
        CONFD_SET_BITBIG(&__tvap->v, (_bufp), (_len));} while(0)
#define CONFD_SET_TAG_ATTR_LIST(_v, _t_tag, _ptr, _size, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                               \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                      \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;              \
        CONFD_SET_LIST(&__tvap->v, (_ptr), (_size));} while(0)
#define CONFD_SET_TAG_ATTR_OBJECTREF(_v, _t_tag, _hkp, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                             \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                    \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;            \
        CONFD_SET_OBJECTREF(&__tvap->v, (_hkp));} while(0)
#define CONFD_SET_TAG_ATTR_OID(_v, _t_tag, _oidp, _attrs, _num_attrs)      \
    do {confd_tag_value_attr_t *__tvap = (_v);                             \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                    \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;            \
        CONFD_SET_OID(&__tvap->v, (_oidp));} while(0)
#define CONFD_SET_TAG_ATTR_IPV4PREFIX(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                               \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                      \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;              \
        CONFD_SET_IPV4PREFIX(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_IPV6PREFIX(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                               \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                      \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;              \
        CONFD_SET_IPV6PREFIX(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_DECIMAL64(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                              \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                     \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;             \
        CONFD_SET_DECIMAL64(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_IDENTITYREF(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                                \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                       \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;               \
        CONFD_SET_IDENTITYREF(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_IPV4_AND_PLEN(_v, _t_tag, _dval,     \
                                         _attrs, _num_attrs)    \
    do {confd_tag_value_attr_t *__tvap = (_v);                  \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);         \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs; \
        CONFD_SET_IPV4_AND_PLEN(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_IPV6_AND_PLEN(_v, _t_tag, _dval,     \
                                         _attrs, _num_attrs)    \
    do {confd_tag_value_attr_t *__tvap = (_v);                  \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);         \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs; \
        CONFD_SET_IPV6_AND_PLEN(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_DQUAD(_v, _t_tag, _dval, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                          \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                 \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;         \
        CONFD_SET_DQUAD(&__tvap->v, (_dval));} while(0)
#define CONFD_SET_TAG_ATTR_HEXSTR(_v, _t_tag, _bufp, _len, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                                 \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                        \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;                \
        CONFD_SET_HEXSTR(&__tvap->v, (_bufp), (_len));} while(0)
#define CONFD_SET_TAG_ATTR_XMLTAG(_v, _t_tag, _ns, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                         \
        __tvap->tag.ns = _ns; __tvap->tag.tag = (_t_tag);              \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;        \
        CONFD_SET_XMLTAG(&__tvap->v, (_t_tag), (_ns));} while(0)
#define CONFD_SET_TAG_ATTR_XMLBEGIN(_v, _t_tag, _ns, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                           \
        __tvap->tag.ns = _ns; __tvap->tag.tag = (_t_tag);                \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;          \
        CONFD_SET_XMLBEGIN(&__tvap->v, (_t_tag), (_ns));} while(0)
#define CONFD_SET_TAG_ATTR_XMLEND(_v, _t_tag, _ns) \
    do {confd_tag_value_attr_t *__tvap = (_v);                         \
        __tvap->tag.ns = _ns; __tvap->tag.tag = (_t_tag);              \
        __tvap->attrs = NULL; __tvap->num_attrs = 0;        \
        CONFD_SET_XMLEND(&__tvap->v, (_t_tag), (_ns));} while(0)
#define CONFD_SET_TAG_ATTR_XMLBEGINDEL(_v, _t_tag, _ns, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                              \
        __tvap->tag.ns = _ns; __tvap->tag.tag = (_t_tag);                   \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;             \
        CONFD_SET_XMLBEGINDEL(&__tvap->v, (_t_tag), (_ns));} while(0)
#define CONFD_SET_TAG_ATTR_NOEXISTS(_v, _t_tag) \
    do {confd_tag_value_attr_t *__tvap = (_v);                      \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);             \
        __tvap->attrs = NULL; __tvap->num_attrs = 0;     \
        CONFD_SET_NOEXISTS(&__tvap->v);} while(0)
#define CONFD_SET_TAG_ATTR_CDBBEGIN(_v, _t_tag, _ns, _index,    \
                                    _attrs, _num_attrs)         \
    do {confd_tag_value_attr_t *__tvap = (_v);                  \
        __tvap->tag.ns = _ns; __tvap->tag.tag = (_t_tag);       \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs; \
        CONFD_SET_CDBBEGIN(&__tvap->v, (_index));} while(0)
#define CONFD_SET_TAG_ATTR_PTR(_v, _t_tag, _type, _valp, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                               \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);                      \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;              \
        CONFD_SET_PTR(&__tvap->v, (_type), (_valp));} while(0)
#define CONFD_SET_TAG_ATTR_DEFAULT(_v, _t_tag, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                     \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);            \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;    \
        CONFD_SET_DEFAULT(&__tvap->v);} while(0)

#define CONFD_SET_TAG_ATTR_VALUE(_v, _t_tag, _vv, _attrs, _num_attrs) \
    do {confd_tag_value_attr_t *__tvap = (_v);                        \
        __tvap->tag.ns = 0; __tvap->tag.tag = (_t_tag);               \
        __tvap->attrs = _attrs; __tvap->num_attrs = _num_attrs;       \
        __tvap->v = *_vv;} while(0)

#define CONFD_SET_TAG_ATTR_NS(_v, _ns)         \
    do {confd_tag_value_attr_t *__tvap = (_v); \
        __tvap->tag.ns = (_ns);} while(0)
...
#define CONFD_ERR_NO_MOUNT_ID               67

/* instance-identifier has stale data after upgrading */
#define CONFD_ERR_STALE_INSTANCE            68
#ifdef __cplusplus
}
#endif
#endif
