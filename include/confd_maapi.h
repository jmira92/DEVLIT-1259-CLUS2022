...
/* The following three flags produce result that can be fetched
 * with maapi_commit_queue_result() */
...
#define MAAPI_COMMIT_NCS_COMMIT_QUEUE_CONTINUE_ON_ERROR       (1 << 21)
#define MAAPI_COMMIT_NCS_COMMIT_QUEUE_ROLLBACK_ON_ERROR       (1 << 22)
#define MAAPI_COMMIT_NCS_COMMIT_QUEUE_STOP_ON_ERROR           (1 << 23)
#define MAAPI_COMMIT_NCS_USE_LSA                              (1 << 19)
#define MAAPI_COMMIT_NCS_NO_LSA                               (1 << 20)
#define MAAPI_COMMIT_NCS_RECONCILE_KEEP_NON_SERVICE_CONFIG    (1 << 24)
#define MAAPI_COMMIT_NCS_RECONCILE_DISCARD_NON_SERVICE_CONFIG (1 << 25)
...
#define MAAPI_CONFIG_NO_BACKQUOTE           (1 << 21)
#define MAAPI_CONFIG_CDB_ONLY               (1 << 22)
#define MAAPI_CONFIG_TURBO_C                (1 << 23)
#define MAAPI_CONFIG_READ_WRITE_ACCESS_ONLY (1 << 25)
...
#define MAAPI_CMD_KEEP_PIPE            (1 << 3)
...
/* Deprecated flags for maapi_shared_xxx() */
...
extern int maapi_ncs_apply_trans_params(int sock, int thandle, int keepopen,
                                        confd_tag_value_t *params, int nparams,
                                        confd_tag_value_t **values,
                                        int *nvalues);

extern int maapi_ncs_get_trans_params(int sock, int thandle,
                                      confd_tag_value_t **values, int *nvalues);

/* Deprecated function - use maapi_ncs_apply_trans_params() instead */
extern int maapi_commit_queue_result(int sock, int thandle, int timeoutsecs,
                                     struct ncs_commit_queue_result *result);

extern int maapi_get_rollback_id(int sock, int thandle, int *fixed_id);
...
extern int maapi_getcwd2(int sock, int thandle, size_t *strsz, char *curdir);
...
extern int maapi_report_progress(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg);

extern int maapi_report_progress2(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg,
    const char *package);

extern unsigned long long maapi_report_progress_start(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg,
    const char *package);

extern int maapi_report_progress_stop(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg,
...
extern int maapi_report_service_progress(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg,
    const char *fmt, ...);

extern int maapi_report_service_progress2(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg,
    const char *package,
...
extern unsigned long long maapi_report_service_progress_start(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg,
    const char *package,
...
extern int maapi_report_service_progress_stop(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    const char *msg,
    const char *annotation,
...
extern int maapi_cs_node_children(int sock, int thandle,
...
extern struct confd_cs_node *maapi_cs_node_cd(int sock, int thandle,
                                              const char *fmt, ...);
...
extern int maapi_load_rollback_fixed(int sock, int thandle, int fixed_num);
...
extern int maapi_xpath2kpath_th(int sock, int thandle,
                                const char *xpath, confd_hkeypath_t **hkp);
...
extern int maapi_cli_diff_cmd2(int sock, int thandle, int thandle_old,
                               char *res, int *size,
                               int flags, const char *fmt, ...);
...
extern int maapi_netconf_ssh_call_home(int sock,
                                       confd_value_t *host,
                                       int port);

extern int maapi_netconf_ssh_call_home_opaque(int sock,
                                              confd_value_t *host,
                                              const char *opaque,
                                              int port);
...
extern int maapi_vncs_write_service_log_entry(int sock, const char *msg,
                                              ...
                                              confd_value_t *level,
                                              const char *fmt, va_list args);
extern unsigned long long maapi_vreport_service_progress(
    int sock, int thandle,
    enum confd_progress_verbosity verbosity,
    enum confd_progress_event_type type,
    const char *msg,
    const char *annotation,
    const char *package,
    unsigned long long timestamp,
    const char *fmt, va_list args);
extern int maapi_vcs_node_children(int sock, int thandle,
                                   struct confd_cs_node *mount_point,
                                   struct confd_cs_node ***children,
                                   int *num_children,
                                   const char *fmt, va_list args);
...
extern int maapi_viterate(
    ...
    void *initstate,
    const char *fmtpath, va_list args);
extern struct confd_cs_node *maapi_vcs_node_cd(int sock, int thandle,
                                               const char *fmt, va_list args);
