...
-define(ECCS, true).
-else.
-define(ECCS, undefined).
-endif.

-ifdef(HAVE_SSL_HONOR_ECC_ORDER).
-define(HONOR_ECC_ORDER, true).
-else.
-define(HONOR_ECC_ORDER, undefined).
-endif.
...
-ifdef(OTP_RELEASE).
-define(ystack(), try throw(1) catch _:_:ST -> ST end).
-define(MAKE_ST(CATCH,STVAR,BODY), CATCH:STVAR -> BODY).
-else.
-define(ystack(),try throw(1) catch _:_ -> (fun erlang:get_stacktrace/0)() end).
