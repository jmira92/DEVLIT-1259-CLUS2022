...
static PyObject *confdValue_as_int(PyConfd_Value_Object *self)
{
    ...
    case C_UINT32: return PyInt_FromLong(CONFD_GET_UINT32(vp));
    ...
    case C_BIT32: return PyInt_FromLong((u_int32_t)CONFD_GET_BIT32(vp));
                ...
                PyObject *kwds)
{
    ...
    case C_DECIMAL64 :
        if (PyTuple_Check(init) && (PyTuple_Size(init) == 2) &&
            ...
            v.value = PyLong_AsLong(PyTuple_GetItem(init, 0));
            v.fraction_digits = PyLong_AsLong(PyTuple_GetItem(init, 1));
                    ...
                    PyObject *args, PyObject *kwds)
{
    ...
    CHECK_CONFD_ERR(confd_str2val(type, value, &v));
        ...
        if (v->type == C_XMLTAG) {
            ...
            while (1) {
                if (key == NULL || key->info.type == NULL ||
                    (tmplen = keyval2str(key->info.type, v, buf, n)) < 0) {
                                               ...
                                               const char *name)
{
    if (strcmp(name, "callpoint") == 0) {
        ...
        return PyInt_FromLong((long)self->tr->attr);
