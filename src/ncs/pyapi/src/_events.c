...
static PyObject *
_events_notifications_connect(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {
...
static PyObject *
_events_notifications_connect2(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {
...
{
    PyObject *slave = PyDict_New();

    PYDICT_SET_ITEM(
            slave, "nodeid", (PyObject*)PyConfd_Value_New_DupTo(&n->nodeid));
    PYDICT_SET_ITEM(slave, "af", PyInt_FromLong(n->af));

    if (n->af == AF_INET) {
        char tmp[INET_ADDRSTRLEN + 1];
        if (inet_ntop(n->af, &n->addr.ip4, tmp, INET_ADDRSTRLEN)) {
            PYDICT_SET_ITEM(slave, "ip4", PyString_FromString(tmp));
        }
        else {
            PYDICT_SET_ITEM(
                    slave, "ip4",
        ...
        if (inet_ntop(n->af, &n->addr.ip6, tmp, INET6_ADDRSTRLEN)) {
            PYDICT_SET_ITEM(slave, "ip6", PyString_FromString(tmp));
        }
        else {
            PYDICT_SET_ITEM(
                    slave, "ip6",
        ...
        if (n->addr.str) {
            PYDICT_SET_ITEM(slave, "str", PyString_FromString(n->addr.str));
        }
        else {
            PyDict_SetItemString(slave, "str", Py_None);
        }
    }

    PYDICT_SET_ITEM(d, name, slave);
}
        ...
        PyObject *d, struct confd_ha_notification *n)
{
    ...
    if (n->type == CONFD_HA_INFO_NOMASTER) {
        PYDICT_SET_ITEM(x, "nomaster", PyInt_FromLong(n->data.nomaster));
    }
    else if (n->type == CONFD_HA_INFO_SLAVE_DIED) {
        add_ha_node_info(x, "slave_died", &n->data.slave_died);
    }
    else if (n->type == CONFD_HA_INFO_SLAVE_ARRIVED) {
        add_ha_node_info(x, "slave_arrived", &n->data.slave_arrived);
    }
    else if (n->type == CONFD_HA_INFO_SLAVE_INITIALIZED) {
        PYDICT_SET_ITEM(x, "cdb_initialized_by_copy",
                             PyInt_FromLong(n->data.cdb_initialized_by_copy));
    }
    else if (n->type == CONFD_HA_INFO_BESLAVE_RESULT) {
        PYDICT_SET_ITEM(x, "beslave_result",
                             PyInt_FromLong(n->data.beslave_result));
    }
...
static PyObject *
_events_read_notification(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {
        "sock",
        NULL
...
static PyObject *
_events_diff_notification_done(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {
        "sock",
        "tctx",
...
static PyObject *
_events_sync_audit_notification(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {
        "sock",
        "usid",
...
static PyObject *
_events_sync_ha_notification(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {
        "sock",
        NULL
