...
class _TransactionBackend(_MaagicBackend):
    ...
    def __init__(self, trans):
        ...
        * trans -- 'maapi.Transaction' object to use
        """
...
class _MaapiBackend(_MaagicBackend):
    ...
    def __init__(self, maapi_o):
        ...
        * maapi_o -- 'maapi.Maapi' object to use
        """
...
class LeafList(Leaf):
    ...
    def set_value(self, value):
        ...
        if value is not None and not hasattr(value, '__iter__'):
            raise MaagicError('Argument must be an iterable')
        if self._backend:
            self.delete()
...
def get_root(backend=None, shared=False):
    ...
    * backend -- Node, Transaction, Maapi object or None
    * shared -- if set to 'True', fastmap-friendly maapi calls, such as
                shared_set_elem, will be used within the returned tree.
    """
