...
static void do_subwait_iter(char *argv[]) /* <path> [prio] [loop] [suspend]*/
{
    int id, n, i, prio, loop, suspend, subids[1];
    if (argv[1]) {
        prio = atoi(argv[1]);
    } else {
        prio = 10;
    ...
    suspend = (argv[3] && strcmp(argv[3], "suspend") == 0) ? 1 : 0;
...
/* Two-phase subscription */
static void do_subwait_iter2p(char *argv[]) /* <path> [prio] [loop] [suspend] */
{
    int id, i, prio, loop;
...
{
    int listener =
        strcmp(argv[0], "netconf") == 0 ? CONFD_LISTENER_NETCONF
...
#define ZERO_OR_MORE_ARGS (-(MAX_ARGS-1))
    ...
    /* CDB subscription */
    {
        ...
        "<path> [priority] [loop] [suspend]",
        "subscribe to <path> and run cdb_diff_iterate() when notified.\n"
        "  If provided [suspend], the subscription will be suspended when it \n"
    ...
    },
    {
        ...
        "  If provided [suspend], where possible values are prepare, commit, or"
        " both, the subscription will be suspended after it receives prepare,"
        " commit, or both of the notifications."
    },
    {
    ...
    },
    {
        ...
        CMD_MAAPI|CMD_MAAPI_NOUSER|CMD_MAAPI_NOTRANS,
        "<listener> = netconf | snmp | cli | webui | netconf_call_home",
...
int main(int argc, char *argv[])
{
    ...
    const char *ops = "da:p:orSIf:F:isUu:x:g:etc:l:mhL?q";
    /* Parse command line */
