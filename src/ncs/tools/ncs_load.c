...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "wbxnNdjF:WSp:P:f:lmrHUaeoAMDiu:g:c:tsOh?"))
           != -1)
    {
        switch(c) {
        case 'w':
        ...
        case 'F':
            ...
            case 't':
            case 'T':
                config_format = MAAPI_CONFIG_TURBO_C;
                break;
            default:
                fatal(0, 1,
                      "invalid format: %s "
                      "(must be one of x, p, o, j, c, i or t)",
                      optarg);
            }
            break;
        case 'W':
        ...
        case 's':
            ...
            break;
        case 'O':
            ldb = sdb = CONFD_OPERATIONAL;
            flags |= MAAPI_CONFIG_OPER_ONLY;
            break;
        case 'b':
            flags |= MAAPI_CONFIG_CDB_ONLY;
            break;
        case 'f':
            filename = optarg;
            break;
        ...
        case 'o':
            ...
            break;
        case 'A':
            flags |= MAAPI_CONFIG_READ_WRITE_ACCESS_ONLY;
            break;
        case 'M':
    ...
    if (config_format == MAAPI_CONFIG_TURBO_C && load_config == 0) {
      fatal(0, 1,
            "format C-style CLI using turbo parser is only applicable for "
            "load config");
    }
    ...
    if (delayed_when) {
      OK(maapi_set_delayed_when(sock, tid, 1));
    }
