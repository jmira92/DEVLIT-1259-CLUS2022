...
static int read_write(int infd, int outfd)
{
    ...
    if ((rval = read(infd, buf, sizeof(buf))) == 0) {
        return 0;
    }
...
static char *get_secondary_gids(gid_t gid, gid_t *groups, int ng)
{
    ...
    for (j = 0; j < ng; j++) {
        if (groups[j] != gid) {
            ...
            snprintf(&buf[len], sizeof(buf) - len, "%d,", groups[j]);
            len += strlen(&buf[len]);
        }
    }
    if (len > 0 && buf[len - 1] == ',') {
...
static void creds(int sock)
{
    ...
    if ((ip = strtok_r(ptr, " ", &saveptr)) == NULL)
        fatal(6, "Badly formatted SSH_CONNECTION environment variable");
    if ((port = strtok_r(NULL, " ", &saveptr)) == NULL)
        port = "0";
    homedir = getenv("HOME");
    user = getenv("USER");
    if (homedir == NULL)
    ...
    if (user == NULL)
        user = "";
    gid = getgid();
    n = getgroups(sizeof(list), list);
    xtragids = get_secondary_gids(gid, list, n);
    snprintf(buf, sizeof(buf), "[%s;%s/%s;ssh;%d;%d;%s;%s;",
            user, ip, port,
            getuid(), gid,
            xtragids, homedir);
    /* extract group information */
    size = sizeof(buf) - strlen(buf) - 3;
    if (n > 0) {
        struct group group;
        struct group *g;
        int i, len;
        long tmpsize = sysconf(_SC_GETGR_R_SIZE_MAX);
        if (tmpsize == -1) {
