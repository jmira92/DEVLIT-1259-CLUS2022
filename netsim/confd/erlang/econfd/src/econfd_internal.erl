...
-spec term_read(Socket, Timeout) -> read_ret() when
...
-spec term_read(Socket, ExpectOp, TimeOut) -> read_ret() when
      Socket :: econfd:socket(),
      ExpectOp :: integer(),
...
-spec get_cs(Ns, Tagpath) -> Result when
      Ns :: econfd:namespace(),
...
-spec load_schemas(Ip, Port) -> 'ok' | {'error', term()} when
      Ip :: econfd:ip(),
      Port :: non_neg_integer().
load_schemas(Address, Port) ->
    gen_server:call(?SERVER, {load, Address, Port}, infinity).

-spec load_schemas(Ip, Port, State) -> Result when
      Ip :: econfd:ip(),
      Port :: non_neg_integer(),
      State :: state(),
        ...
        {ok, Term, Acc1} ->
            case Term of
                {node,
                 ...
                 Children, Keys, Flags, Actions, Cmp, Notifs,
                 MetaData, MountId} ->
...
mk_facet({display_hint, Value}) ->
    DH =
        case snmp_display_hint:compile_octet_string_dh(?b2l(Value)) of
            {ok, D} -> D;
            _ -> undefined
        end,
    #display_hint{value = Value, compiled = DH};
...
%% Ns and TypeName must refer to matching qualified namespace and type
%% in NsList.
-spec value2string(NsList, Type, Value, FmtType) -> Result when
      NsList :: saxlite:ns_list(),
      Type :: {Ns, TypeName},
      Ns :: econfd:namespace(),
      TypeName :: atom(),
...
-spec check_val(Type, Value) -> Result when
      Type :: confd_cs_node() | econfd:type(),
      Value :: econfd:value(),
      Result :: {'ok', Lexical} | {'error', term()},
      Lexical :: binary().
...
schema_handler('AESCFB128IVEncryptedString') ->
    schema_handler(string);
schema_handler('AES256CFB128IVEncryptedString') ->
    schema_handler(string);
...
schema_handler('aes-cfb-128-encrypted-string') ->
    schema_handler('AESCFB128IVEncryptedString');
schema_handler('aes-256-cfb-128-encrypted-string') ->
    schema_handler('AES256CFB128IVEncryptedString');
...
decimal_string(Value, TypeStack) ->
    ...
    AbsVal = abs(Value),
    DecPos = myfloor(math:log10(AbsVal)) + 1,
...
myfloor(V) when V >= 0 -> trunc(V);
myfloor(V) -> trunc(V - trunc(V) + 1) + trunc(V) - 1.
...
base64Binary_value(#exs_ctx{type_stack=TypeStack}, String) ->
    case get_display_hint(TypeStack) of
        #display_hint{compiled = DH} ->
            Collapsed = ws_collapse(String),
            case snmp_display_hint:string_to_octet_string(Collapsed, DH) of
                {ok, Octets} ->
                    {?C_BINARY, ?l2b(Octets)};
                _ ->
                    {?C_BINARY, base64:encode(String)}
            end;
        false ->
            {?C_BINARY, base64:decode(String)}
    end.

base64Binary_string({?C_BINARY, Binary}, TypeStack) ->
    case get_display_hint(TypeStack) of
        #display_hint{compiled = DH} ->
            ?l2b(snmp_display_hint:octet_string_to_string(?b2l(Binary), DH));
        false ->
...
get_display_hint([#exs_type{derivation=#restriction{facets=Facets}}|Rest]) ->
    get_display_hint(lists:append(Facets, Rest));
get_display_hint([#display_hint{} = DH | _]) ->
    DH;
get_display_hint([_ | Rest]) ->
    get_display_hint(Rest);
get_display_hint([]) ->
    false.
