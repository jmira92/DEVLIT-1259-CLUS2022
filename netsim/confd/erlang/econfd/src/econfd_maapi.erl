...
%% @equiv shared_create(Socket, Tid, true, IKeypath)
...
%%
%% Update Back Pointer if BackP is true. Used by NCS Fastmap code.
%%
%% This function is deprecated, use the shared_create/3 function.
-spec shared_create(Socket, Tid, BackP, IKeypath) -> 'ok' | err() when
      Socket :: econfd:socket(),
      Tid :: integer(),
      BackP :: boolean(),
      IKeypath :: econfd:ikeypath().
...
shared_set_values(Sock, Tid, IKP, ValueList) ->
    shared_set_values(Sock, Tid, IKP, ValueList, true).

%% @doc Write a list of tagged values from NCS FastMap.
%%
%% Update backpointer if CreateBackpointer is true.
%%
%% This function is deprecated, use the shared_set_values/4 function.
-spec shared_set_values(Socket, Tid, IKeypath, ValueList, CreateBackpointer) ->
          'ok' | err() when
      Socket :: econfd:socket(),
      Tid :: integer(),
      IKeypath :: econfd:ikeypath(),
...
-spec ncs_apply_template(Socket, Tid, TemplateName, RootIKeypath, Variables,
                         Documents, Shared, Backpointer) -> 'ok' | err() when
      Socket :: econfd:socket(),
      Tid :: integer(),
      TemplateName :: binary(),
