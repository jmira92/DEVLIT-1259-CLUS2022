...
%% flags for confd_data_cbs registration
-define(CONFD_DATA_WANT_FILTER, (1 bsl 0)).
...
-record(confd_data_cbs, {
          ...
          get_case          :: 'undefined' | econfd:cb_get_case(),
          set_case          :: 'undefined' | econfd:cb_set_case(),
...
-record(ncs_service_cbs, {
          ...
          %% Pre-lock-create is deprecated, and only present for temporary
...
-record(confd_crypto_keys, {
          ...
          aes256_key,  %% binary(32)
          aes256_keys_initialized = false%% bool()
...
-record(confd_list_filter, {
          type :: econfd:list_filter_type(),
          expr1 = undefined :: 'undefined' | #confd_list_filter{},
          expr2 = undefined :: 'undefined' | #confd_list_filter{},
          op = undefined :: 'undefined' | econfd:list_filter_op(),
          node = undefined :: 'undefined' | econfd:ikeypath(),
          val = undefined :: 'undefined' | econfd:value()}).
...
-define(C_XMLMOVEFIRST, 51). %% OBU list move-first marker
-define(C_XMLMOVEAFTER, 52). %% OBU list move-after marker
...
-define(CONFD_ATTR_ORIGIN,     16#80000007).
...
%% flags for confd_data_cbs callbacks
-define(CONFD_TRANS_CB_FLAG_FILTERED, (1 bsl 0)).
...
-record(confd_trans_ctx, {
          ...
          traversal_id,         %%  integer() unique get_next traversal id
          list_filter,          %%  list filter if registered with
                                %%  CONFD_DATA_WANT_FILTER and filter used
          cb_flags = 0,         %%  callback return flags
...
-define(CDB_ITER_WANT_CLI_ORDER,       (1 bsl 10)).
...
-define(CONFD_NOTIF_PROGRESS,         (1 bsl 24)).
-define(NCS_NOTIF_CALL_HOME_INFO,     (1 bsl 25)).
-define(CONFD_NOTIF_JSONRPC,          (1 bsl 26)).
-define(CONFD_NOTIF_WEBUI,            (1 bsl 27)).
...
-define(CONFD_PROGRESS_START, 1).
-define(CONFD_PROGRESS_STOP,  2).
-define(CONFD_PROGRESS_INFO,  3).

-record(econfd_notif_progress, {
          type,            %% integer() START | STOP | INFO
          timestamp,       %% integer() Timestamp in microseconds since Epoch
          duration,        %% integer() Duration of event in microseconds
          usid,            %% integer() User session id
          tid,             %% integer() Transaction id
          database,        %% integer() NO_DB | CANDIDATE | RUNNING | STARTUP |
                           %%           OPERATIONAL | PRE_COMMMIT_RUNNING |
                           %%           INTENDED
          context,         %% binary()  Session context
          trace_id,        %% binary()  Per request unique trace id
          subsystem,       %% binary()  Name of the subsystem
          phase,           %% binary()  Phase of the transaction
          msg,             %% binary()  Progress event message
          annotation,      %% binary()  Metadata about the event, indicating
                           %%           error or explains latency etc.
          service,         %% binary()  The invoked service instance
          service_phase,   %% binary()  Callback phase of the invoked service
          commit_queue_id, %% integer() Commit queue item id
          node,            %% binary()  Remote node name
          device,          %% binary()  Device name
          device_phase,    %% binary()  Device communication phase
          package          %% binary()  The package from which the event
                           %%           originates
         }).
...
-define(CONFD_CS_HAS_MOUNT_POINT,          (1 bsl 15)).
-define(CONFD_CS_NODE_IS_STRING_AS_BINARY, (1 bsl 16)).
...
-define(liof_bt(Fmt, Args), io:format(user, "~w:~w ~s ~p\n",
                             [?MODULE, ?LINE,
                              io_lib:format(Fmt, Args), ?confdstack()])).
-endif.
