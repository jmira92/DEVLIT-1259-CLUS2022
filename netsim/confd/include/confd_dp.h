...
/* Keep in sync with macros defined in ietf-yang-push.hrl
 * yp_create, yp_delete, yp_insert, yp_merge ... */
enum confd_data_op {
    CONFD_DATA_CREATE = 0,
    CONFD_DATA_DELETE = 1,
    CONFD_DATA_INSERT = 2,
...
#define CONFD_DATA_EDIT() (struct confd_data_edit) {\
    .edit_id = NULL,\
    .op = CONFD_DATA_CREATE,\
    .target = NULL,\
    .point = NULL,\
    .where = CONFD_DATA_BEFORE,\
    .data = NULL,\
    .ndata = 0,\
    .flags = 0,\
    .set_path = &set_patch_edit_path\
}

#define CONFD_DATA_EDIT_SET_PATH(x, y, ...) \
    x->set_path(x, offsetof(struct confd_data_edit, y), __VA_ARGS__)
...
struct confd_data_edit {
    ...
    int ndata;
    int flags;
    int (*set_path)(const struct confd_data_edit *edit,
                    size_t offset,
                    const char *fmt, ...);
};

/* Keep in sync with yang_push_handler.erl*/
#define CONFD_PATCH_FLAG_INCOMPLETE             (1 << 0)
#define CONFD_PATCH_FLAG_BUFFER_DAMPENED        (1 << 1)
#define CONFD_PATCH_FLAG_FILTER                 (1 << 2)
#define CONFD_PATCH_FLAG_AAA_CHECKED            (1 << 3)


#define CONFD_DATA_PATCH() (struct confd_data_patch) {\
    .patch_id = NULL,\
    .comment = NULL,\
    .edits = NULL,\
    .nedits = 0,\
...
/* flags for confd_data_cbs.flags */
#define CONFD_DATA_WANT_FILTER          (1 << 0)
...
struct ncs_service_cbs {
    ...
    int (*pre_modification)(struct confd_trans_ctx *tctx,
                            ...
                            int num_props);
    /* This member is deprecated, and only present for temporary backward
...
#define CONFD_ACCESS_CHK_DESCENDANT       (1 << 10)
...
struct confd_daemon_ctx {
    ...
    struct confd_push_on_change_cbs *push_cbs;
    int push_cbs_len, num_push_cbs;
...
extern int confd_data_reply_value_attrs(struct confd_trans_ctx *tctx,
                                        const confd_value_t *v,
...
extern int confd_data_reply_tag_value_attrs_array(
...
extern int confd_data_reply_next_key_attrs(struct confd_trans_ctx *tctx,
                                           const confd_value_t *v,
                                           int num_vals_in_key,
                                           long next,
                                           const confd_attr_value_t *attrs,
...
extern int confd_data_reply_next_object_tag_value_attrs_array(
    struct confd_trans_ctx *tctx,
...
extern int confd_data_reply_next_object_tag_value_attrs_arrays(
    struct confd_trans_ctx *tctx,
...
/* START - YANG-Push Callbacks */
extern int confd_register_push_on_change(
    struct confd_daemon_ctx *dx,
    const struct confd_push_on_change_cbs *pcbs);

extern int confd_push_on_change(
        struct confd_push_on_change_ctx *pctx,
        struct confd_datetime *time,
        const struct confd_data_patch *patch);

int set_patch_edit_path(const struct confd_data_edit *edit, size_t offset,
                        const char *fmt, ...);
/* END - YANG-Push Callbacks */
...
extern int confd_data_get_list_filter(struct confd_trans_ctx *tctx,
                                      struct confd_list_filter **filter);

extern void confd_free_list_filter(struct confd_list_filter *filter);
...
extern int confd_notification_send_path(
    struct confd_notification_ctx *nctx,
    struct confd_datetime *time,
    confd_tag_value_t *values, int nvalues,
    const char *fmt, ...);
