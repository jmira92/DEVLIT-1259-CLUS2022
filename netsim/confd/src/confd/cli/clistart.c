...
static int write_fill_confd(int fd, char *buf, int len)
{
    ...
    for(i=0, n=0 ; i < len ; i++, n++) {
        ...
        if (n >= out0buf_len) {
            ...
            new_out0buf = (char*) malloc(out0buf_len*2);
            if (new_out0buf == NULL) {
                fprintf(stderr, "Error: failed to allocate memory\n");
                exit(1);
            }

            memcpy(new_out0buf, out0buf, out0buf_len);

            free(out0buf);

            out0buf = new_out0buf;
            out0buf_len = out0buf_len*2;
        }
        if (buf[i] == 0) {
            out0buf[n++] = 0;
        }
...
int main(int argc, char *argv[])
{
    ...
    out0buf = (char *) malloc(out0buf_len);
    if (out0buf == NULL) {
    ...
    /* determine user name */
    if (user == NULL) {
        tmpsize = MAX(sysconf(_SC_LOGIN_NAME_MAX),
                      sysconf(_SC_GETPW_R_SIZE_MAX));
        if (tmpsize == -1) {
            tmpsize = 4096; /* really huge (from Mac) */
        }
        char tmp[tmpsize];
        if (getlogin_r(tmp, sizeof(tmp)) == 0) {
            user = tmp;
        } else {
            user = NULL;
        }
        ...
        if (!getpwnam_r(user, &passwd, tmp, sizeof(tmp), &pwd) && pwd &&
            pwd->pw_uid != getuid()) {
            if (!getpwuid_r(getuid(), &passwd, tmp, sizeof(tmp), &pwd) &&
                pwd) {
                user = pwd->pw_name;
            }
        }
    ...
    if (groups == NULL) {
        ...
        if ((n = getgroups(n, list)) > 0) {
            ...
            for(i=0 ; i < n ; i++) {
                if (!getgrgid_r(list[i], &group, tmp, sizeof(tmp), &g) && g) {
