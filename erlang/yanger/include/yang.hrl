...
-record(yctx, {
          ...
          %% Prefix namespace maps of annotating modules, used to compile
          %% statements if these statements are annotated from annotating
          %% modules
          annotation_ns_maps = yang:map_new() :: yang:map0(),
          ...
          %% Imports that are only used in annotations
          annotation_imports = yang:map_new() :: yang:map0(),
          ...
          pmap = yang:map_new() :: yang:map0(), % used by plugins
          allow_interop_issues = false :: boolean(),
