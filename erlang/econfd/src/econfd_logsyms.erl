...
get_logsym(?LOGSYM_BIND_ERR) -> {"BIND_ERR","NCS failed to bind to one of the internally used listen sockets."};
...
get_logsym(?LOGSYM_EXT_BIND_ERR) -> {"EXT_BIND_ERR","NCS failed to bind to one of the externally visible listen sockets."};
get_logsym(?LOGSYM_ACCEPT_FDLIMIT) -> {"ACCEPT_FDLIMIT","NCS failed to accept a connection due to reaching the process or system-wide file descriptor limit."};
get_logsym(?LOGSYM_ACCEPT_FATAL) -> {"ACCEPT_FATAL","NCS encountered an OS-specific error indicating that networking support is unavailable."};
...
get_logsym(?LOGSYM_NO_CALLPOINT) -> {"NO_CALLPOINT","NCS tried to populate an XML tree but no code had registered under the relevant callpoint."};
...
get_logsym(?LOGSYM_INTERNAL_ERROR) -> {"INTERNAL_ERROR","A NCS internal error - should be reported to support@tail-f.com."};
...
get_logsym(?LOGSYM_BRIDGE_DIED) -> {"BRIDGE_DIED","NCS is configured to start the confd_aaa_bridge and the C program died."};
get_logsym(?LOGSYM_PHASE0_STARTED) -> {"PHASE0_STARTED","NCS has just started its start phase 0."};
get_logsym(?LOGSYM_PHASE1_STARTED) -> {"PHASE1_STARTED","NCS has just started its start phase 1."};
get_logsym(?LOGSYM_STARTED) -> {"STARTED","NCS has started."};
...
get_logsym(?LOGSYM_CONSULT_FILE) -> {"CONSULT_FILE","NCS is reading its configuration file."};
get_logsym(?LOGSYM_STOPPING) -> {"STOPPING","NCS is stopping (due to e.g. ncs --stop)."};
get_logsym(?LOGSYM_RELOAD) -> {"RELOAD","Reload of daemon configuration has been initiated."};
get_logsym(?LOGSYM_BADCONFIG) -> {"BADCONFIG","ncs.conf contained bad data."};
...
get_logsym(?LOGSYM_LISTENER_INFO) -> {"LISTENER_INFO","NCS starts or stops to listen for incoming connections."};
get_logsym(?LOGSYM_NETCONF_HDR_ERR) -> {"NETCONF_HDR_ERR","The cleartext header indicating user and groups was badly formatted."};
get_logsym(?LOGSYM_LIB_BAD_VSN) -> {"LIB_BAD_VSN","An application connecting to NCS used a library version that doesn't match the NCS version (e.g. old version of the client library)."};
get_logsym(?LOGSYM_LIB_BAD_SIZES) -> {"LIB_BAD_SIZES","An application connecting to NCS used a library version that can't handle the depth and number of keys used by the data model."};
get_logsym(?LOGSYM_LIB_NO_ACCESS) -> {"LIB_NO_ACCESS","Access check failure occurred when an application connected to NCS."};
...
get_logsym(?LOGSYM_SNMP_TRAP_UNKNOWN_SENDER) -> {"SNMP_TRAP_UNKNOWN_SENDER","An SNMP trap was to be forwarded, but the sender was not listed in ncs.conf."};
...
get_logsym(?LOGSYM_MISSING_DES3CBC_SETTINGS) -> {"MISSING_DES3CBC_SETTINGS","DES3CBC keys were not found in ncs.conf"};
get_logsym(?LOGSYM_MISSING_AESCFB128_SETTINGS) -> {"MISSING_AESCFB128_SETTINGS","AESCFB128 keys were not found in ncs.conf"};
...
get_logsym(?LOGSYM_DEVEL_CONFD) -> {"DEVEL_CONFD","Developer NCS log message"};
...
get_logsym(?LOGSYM_COMMIT_UN_SYNCED_DEV) -> {"COMMIT_UN_SYNCED_DEV",  "Data was committed toward a device with bad or unknown sync state"};
get_logsym(?LOGSYM_NCS_SNMP_INIT_ERR) -> {"NCS_SNMP_INIT_ERR","Failed to locate snmp_init.xml in loadpath"};
get_logsym(?LOGSYM_NCS_JAVA_VM_START) -> {"NCS_JAVA_VM_START","Starting the NCS Java VM"};
get_logsym(?LOGSYM_NCS_JAVA_VM_FAIL) -> {"NCS_JAVA_VM_FAIL","The NCS Java VM failure/timeout"};
get_logsym(?LOGSYM_NCS_PACKAGE_SYNTAX_ERROR) -> {"NCS_PACKAGE_SYNTAX_ERROR","Syntax error in package file"};
get_logsym(?LOGSYM_NCS_PACKAGE_DUPLICATE) -> {"NCS_PACKAGE_DUPLICATE","Duplicate package found"};
get_logsym(?LOGSYM_NCS_PACKAGE_COPYING) -> {"NCS_PACKAGE_COPYING","A package is copied from the load path to private directory"};
get_logsym(?LOGSYM_NCS_PACKAGE_UPGRADE_ABORTED) -> {"NCS_PACKAGE_UPGRADE_ABORTED","The CDB upgrade was aborted implying that CDB is untouched. However the package state is changed"};
get_logsym(?LOGSYM_NCS_PACKAGE_BAD_NCS_VERSION) -> {"NCS_PACKAGE_BAD_NCS_VERSION","Bad NCS version for package"};
get_logsym(?LOGSYM_NCS_PACKAGE_BAD_DEPENDENCY) -> {"NCS_PACKAGE_BAD_DEPENDENCY","Bad NCS package dependency"};
get_logsym(?LOGSYM_NCS_PACKAGE_CIRCULAR_DEPENDENCY) -> {"NCS_PACKAGE_CIRCULAR_DEPENDENCY","Circular NCS package dependency"};
...
get_logsym(?LOGSYM_RESERVED_110) -> {"RESERVED_110","RESERVED_110"};
get_logsym(?LOGSYM_RESERVED_111) -> {"RESERVED_111","RESERVED_111"};
get_logsym(?LOGSYM_RESERVED_112) -> {"RESERVED_112","RESERVED_112"};
get_logsym(?LOGSYM_RESERVED_113) -> {"RESERVED_113","RESERVED_113"};
get_logsym(?LOGSYM_RESERVED_114) -> {"RESERVED_114","RESERVED_114"};
get_logsym(?LOGSYM_RESERVED_115) -> {"RESERVED_115","RESERVED_115"};
...
get_logsym(?LOGSYM_RESERVED_119) -> {"RESERVED_119","RESERVED_119"};
get_logsym(?LOGSYM_RESERVED_120) -> {"RESERVED_120","RESERVED_120"};
get_logsym(?LOGSYM_RESERVED_121) -> {"RESERVED_121","RESERVED_121"};
get_logsym(?LOGSYM_NOAAA_CLI_LOGIN) -> {"NOAAA_CLI_LOGIN","A user used the --noaaa flag to ncs_cli"};
get_logsym(?LOGSYM_RESERVED_123) -> {"RESERVED_123","RESERVED_123"};
get_logsym(?LOGSYM_RESERVED_124) -> {"RESERVED_124","RESERVED_124"};
...
get_logsym(?LOGSYM_NCS_DEVICE_OUT_OF_SYNC) -> {"NCS_DEVICE_OUT_OF_SYNC","A check-sync action reported out-of-sync for a device"};
get_logsym(?LOGSYM_NCS_SERVICE_OUT_OF_SYNC) -> {"NCS_SERVICE_OUT_OF_SYNC","A check-sync action reported out-of-sync for a service"};
get_logsym(?LOGSYM_NCS_PYTHON_VM_START) -> {"NCS_PYTHON_VM_START","Starting the named NCS Python VM"};
get_logsym(?LOGSYM_NCS_PYTHON_VM_FAIL) -> {"NCS_PYTHON_VM_FAIL","The NCS Python VM failure/timeout"};
get_logsym(?LOGSYM_NCS_SET_PLATFORM_DATA_ERROR) -> {"NCS_SET_PLATFORM_DATA_ERROR","The device failed to set the platform operational data at connect"};
get_logsym(?LOGSYM_NCS_SMART_LICENSING_START) -> {"NCS_SMART_LICENSING_START","Starting the NCS Smart Licensing Java VM"};
get_logsym(?LOGSYM_NCS_SMART_LICENSING_FAIL) -> {"NCS_SMART_LICENSING_FAIL","The NCS Smart Licensing Java VM failure/timeout"};
get_logsym(?LOGSYM_NCS_SMART_LICENSING_GLOBAL_NOTIFICATION) -> {"NCS_SMART_LICENSING_GLOBAL_NOTIFICATION","Smart Licensing Global Notification"};
get_logsym(?LOGSYM_NCS_SMART_LICENSING_ENTITLEMENT_NOTIFICATION) -> {"NCS_SMART_LICENSING_ENTITLEMENT_NOTIFICATION","Smart Licensing Entitlement Notification"};
get_logsym(?LOGSYM_NCS_SMART_LICENSING_EVALUATION_COUNTDOWN) -> {"NCS_SMART_LICENSING_EVALUATION_COUNTDOWN","Smart Licensing evaluation time remaining"};
...
get_logsym(?LOGSYM_REST_AUTH_SUCCESS) -> {"REST_AUTH_SUCCESS","A rest authenticated user logged in."};
get_logsym(?LOGSYM_REST_AUTH_FAIL) -> {"REST_AUTH_FAIL","Rest authentication for a user failed."};
get_logsym(?LOGSYM_STARTING) -> {"STARTING","NCS is starting."};
get_logsym(?LOGSYM_NCS_PACKAGE_UPGRADE_UNSAFE) -> {"NCS_PACKAGE_UPGRADE_UNSAFE","Package upgrade has been aborted due to warnings."};
...
get_logsym(?LOGSYM_AUTH_LOGIN_SUCCESS) -> {"AUTH_LOGIN_SUCCESS","A user logged into NCS."};
get_logsym(?LOGSYM_AUTH_LOGIN_FAIL) -> {"AUTH_LOGIN_FAIL","A user failed to log in to NCS."};
get_logsym(?LOGSYM_AUTH_LOGOUT) -> {"AUTH_LOGOUT","A user was logged out from NCS."};
get_logsym(?LOGSYM_SESSION_CREATE) -> {"SESSION_CREATE","A new user session was created"};
get_logsym(?LOGSYM_SESSION_TERMINATION) -> {"SESSION_TERMINATION","A user session was terminated due to specified reason"};
get_logsym(?LOGSYM_SESSION_MAX_EXCEEDED) -> {"SESSION_MAX_EXCEEDED","A user failed to create a new user sessions due to exceeding sessions limits"};
get_logsym(?LOGSYM_ABORT_CAND_COMMIT_REBOOT) -> {"ABORT_CAND_COMMIT_REBOOT","NCS restarted while having a ongoing candidate commit timer, reverting configuration."};
get_logsym(?LOGSYM_CAND_COMMIT_ROLLBACK_FAILURE) -> {"CAND_COMMIT_ROLLBACK_FAILURE","Failed to rollback candidate commit"};
get_logsym(?LOGSYM_CAND_COMMIT_ROLLBACK_DONE) -> {"CAND_COMMIT_ROLLBACK_DONE","Candidate commit rollback done"};
get_logsym(?LOGSYM_JSONRPC_LOG_MSG) -> {"JSONRPC_LOG_MSG","JSON-RPC traffic log message"};
get_logsym(?LOGSYM_JSONRPC_WARN_MSG) -> {"JSONRPC_WARN_MSG","JSON-RPC warning message"};
get_logsym(?LOGSYM_REST_REQUEST) -> {"REST_REQUEST","REST request"};
get_logsym(?LOGSYM_REST_RESPONSE) -> {"REST_RESPONSE","REST response"};
get_logsym(?LOGSYM_RESTCONF_REQUEST) -> {"RESTCONF_REQUEST","RESTCONF request"};
get_logsym(?LOGSYM_RESTCONF_RESPONSE) -> {"RESTCONF_RESPONSE","RESTCONF response"};
get_logsym(?LOGSYM_MISSING_AES256CFB128_SETTINGS) -> {"MISSING_AES256CFB128_SETTINGS","AES256CFB128 keys were not found in ncs.conf"};
get_logsym(?LOGSYM_NCS_PYTHON_VM_START_UPGRADE) -> {"NCS_PYTHON_VM_START_UPGRADE","Starting a Python VM to run upgrade code"};
get_logsym(?LOGSYM_NCS_SNMPM_START) -> {"NCS_SNMPM_START","Starting the NCS SNMP manager component"};
get_logsym(?LOGSYM_NCS_SNMPM_STOP) -> {"NCS_SNMPM_STOP","The NCS SNMP manager component has been stopped"};
get_logsym(?LOGSYM_WEBUI_LOG_MSG) -> {"WEBUI_LOG_MSG","WebUI access log message"};
get_logsym(?LOGSYM_LOCAL_AUTH_FAIL_NOUSER) -> {"LOCAL_AUTH_FAIL_NOUSER","Authentication for a locally configured user failed due to user not found."};
get_logsym(?LOGSYM_LOCAL_AUTH_FAIL_BADPASS) -> {"LOCAL_AUTH_FAIL_BADPASS","Authentication for a locally configured user failed due to providing bad password."};
get_logsym(?LOGSYM_EXT_AUTH_TOKEN_SUCCESS) -> {"EXT_AUTH_TOKEN_SUCCESS","An externally token authenticated user logged in."};
get_logsym(?LOGSYM_EXT_AUTH_TOKEN_FAIL) -> {"EXT_AUTH_TOKEN_FAIL","External token authentication failed for a user."};
get_logsym(?LOGSYM_EXT_AUTH_2FA_SUCCESS) -> {"EXT_AUTH_2FA_SUCCESS","An external challenge authenticated user logged in."};
get_logsym(?LOGSYM_EXT_AUTH_2FA_FAIL) -> {"EXT_AUTH_2FA_FAIL","External challenge authentication failed for a user."};
get_logsym(?LOGSYM_EXT_AUTH_2FA) -> {"EXT_AUTH_2FA","External challenge sent to a user."};
get_logsym(?LOGSYM_CONFIG_CHANGE) -> {"CONFIG_CHANGE","A change to NCS configuration has taken place, e.g., by a reload of the configuration file"};
get_logsym(?LOGSYM_DUPLICATE_NAMESPACE) -> {"DUPLICATE_NAMESPACE",   "Duplicate namespace found."};
get_logsym(?LOGSYM_EXEC_WHEN_CIRCULAR_DEPENDENCY) -> {"EXEC_WHEN_CIRCULAR_DEPENDENCY","An error occurred while evaluating a when-expression."};
get_logsym(?LOGSYM_ROLLBACK_FAIL_DELETE) -> {"ROLLBACK_FAIL_DELETE","Failed to delete rollback file."};
